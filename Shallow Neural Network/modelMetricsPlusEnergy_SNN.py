...
def build_runtime_cmd(runtime: str, root: Path, wasm: Path, cwasm: Path | None, wasm_args: list[str]) -> list[str]:
    ...
    if runtime == "wasmtime_jit":
        wasmtime = resolve_runtime_binary(
            [home / ".wasmtime/bin/wasmtime", "wasmtime"], "wasmtime"
        )
    ...
    if runtime in ("wamr_interp", "wamr_fast_interp", "wamr_iwasm", "wamr_aot"):
        ...
        if runtime == "wamr_aot":
            aot_candidates = [
                root / "ml_wasi_snn_wamr.aot",
                root / "ml_wasi_snn.aot",
                root / "mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_snn.aot",
                root / "mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_snn_wamr.aot",
            ]
            aot = first_existing(aot_candidates)
            if aot is None:
                raise SystemExit(
                    "WAMR AOT file not found. Build it first with wamrc and rerun wamr_aot."
                )
        ...
        # wamr_interp and wamr_iwasm both map to plain interpreter mode here.
        return [iwasm, f"--map-dir=/data::{str((root / 'data').resolve())}", str(wasm)] + wasm_args

    if runtime == "wasm3":
        wasm3 = resolve_runtime_binary(["wasm3", "m3"], "wasm3")
        # Commonly used wasm3 form supports --dir /work::host
        return [wasm3, "--dir", f"/work::{root}", str(wasm), "--"] + wasm_args

    if runtime == "native":
        native_candidates = [
            root / "ml_native_snn",
            root / "target/release/ml_native_snn",
            root / "mlRust-wasi/target/release/ml_native_snn",
            root / "mlRust-native/target/release/ml_native_snn",
        ]
...
def main():
    ...
    ap.add_argument("--wasm", type=str, default="mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_snn.wasm")
    ap.add_argument("--cwasm", type=str, default="ml_wasi_snn.cwasm")
    args = ap.parse_args()

    root = Path.cwd().resolve()
    wasm = (root / args.wasm).resolve() if not Path(args.wasm).is_absolute() else Path(args.wasm).resolve()
    cwasm = (root / args.cwasm).resolve() if not Path(args.cwasm).is_absolute() else Path(args.cwasm).resolve()
    if not wasm.exists() and args.runtime != "native":
