...
def build_runtime_cmd(runtime: str, root: Path, wasm: Path, cwasm: Path | None, wasm_args: list[str]) -> list[str]:
    ...
    if runtime in ("wasmedge", "wasmedge_aot"):
        ...
        so_candidates = [
            root / "ml_wasi_nb.so",
            root / "target/wasm32-wasip1/release/ml_wasi_nb.so",
            root / "mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_nb.so",
        ]
    ...
    if runtime in ("wamr_interp", "wamr_fast_interp", "wamr_iwasm", "wamr_aot"):
        ...
        if runtime == "wamr_aot":
            aot_candidates = [
                root / "ml_wasi_nb_wamr.aot",
                root / "ml_wasi_nb.aot",
                root / "target/wasm32-wasip1/release/ml_wasi_nb.aot",
                root / "target/wasm32-wasip1/release/ml_wasi_nb_wamr.aot",
                root / "mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_nb.aot",
                root / "mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_nb_wamr.aot",
            ]
    ...
    if runtime == "native":
        native_candidates = [
            root / "ml_native_nb",
            root / "target/release/ml_native_nb",
            root / "mlRust-wasi/target/release/ml_native_nb",
            root / "mlRust-native/target/release/ml_native_nb",
            root / "target/release/ml_wasi_nb",
        ]
...
def resolve_input_artifacts(root: Path, args) -> tuple[Path, Path]:
    ...
    wasm_candidates += [
        root / "ml_wasi_nb.wasm",
        root / "target/wasm32-wasip1/release/ml_wasi_nb.wasm",
        root / "mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_nb.wasm",
    ]
    wasm = first_existing([p.resolve() for p in wasm_candidates if p is not None])
    if wasm is None and args.runtime != "native":
        raise SystemExit("Naive Bayes WASM not found. Checked: " + ", ".join(str(p) for p in wasm_candidates))
    ...
    cwasm_candidates += [
        root / "ml_wasi_nb.cwasm",
        root / "target/wasm32-wasip1/release/ml_wasi_nb.cwasm",
        root / "mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_nb.cwasm",
    ]
    cwasm = first_existing([p.resolve() for p in cwasm_candidates if p is not None])
    if cwasm is None:
        cwasm = (root / (args.cwasm or "ml_wasi_nb.cwasm")).resolve()
...
def main():
    ...
    ap.add_argument("--wasm", type=str, default="ml_wasi_nb.wasm")
    ap.add_argument("--cwasm", type=str, default="ml_wasi_nb.cwasm")
    args = ap.parse_args()
    ...
    common_args = [
        ...
        "--var-smoothing", str(args.var_smoothing),
    ]
    ...
    while good_runs < max_runs and attempt < max_runs * 5:
        ...
        if ts_min > (t_start + METER_COVERAGE_TOLERANCE_S) or ts_max < (t_end - METER_COVERAGE_TOLERANCE_S):
            print(
                ...
                f"(ts_min={ts_min:.3f}, ts_max={ts_max:.3f}, "
                f"t_start={t_start:.3f}, t_end={t_end:.3f}). Retrying.",
                flush=True,
            )
            time.sleep(args.cooldown_s)
            continue
        ...
        if good_runs >= min_runs:
            ...
            print(
                f"[{args.runtime}] precision check: n={good_runs} "
                f"relCI_halfwidth={rel_hw:.4f} target={target:.4f}",
                flush=True,
            )
            if rel_hw <= target:
    ...
    print("\n=== FINAL OUTPUT ===")
    print(f"Execution Time (s): {final['execution_time_s']:.6f}")
    print(f"Dynamic Energy (J): {final['dynamic_energy_j']:.6f}")
    print(f"Accuracy: {final['accuracy']:.6f}")
    print(f"Precision: {final['precision']:.6f}")
    print(f"Recall: {final['recall']:.6f}")
    print(f"F1 Score: {final['f1_score']:.6f}")
