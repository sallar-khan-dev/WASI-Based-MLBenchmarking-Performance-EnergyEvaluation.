...
def build_runtime_cmd(runtime: str, root: Path, wasm: Path, cwasm: Path | None, wasm_args: list[str]) -> list[str]:
    ...
    if runtime in ("wamr_interp", "wamr_fast_interp", "wamr_iwasm", "wamr_aot"):
        ...
        if runtime == "wamr_aot":
            aot_candidates = [
                root / "ml_wasi_kmeans_wamr.aot",
                root / "ml_wasi_kmeans.aot",
                root / "target/wasm32-wasip1/release/ml_wasi_kmeans.aot",
                root / "target/wasm32-wasip1/release/ml_wasi_kmeans_wamr.aot",
                root / "mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_kmeans.aot",
                root / "mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_kmeans_wamr.aot",
            ]
    ...
    if runtime == "native":
        native_candidates = [
            root / "ml_native_kmeans",
            root / "target/release/ml_native_kmeans",
            root / "mlRust-wasi/target/release/ml_native_kmeans",
            root / "mlRust-native/target/release/ml_native_kmeans",
            root / "target/release/ml_wasi_kmeans",
        ]
...
def resolve_input_artifacts(root: Path, args) -> tuple[Path, Path]:
    ...
    wasm_candidates += [
        root / "ml_wasi_kmeans.wasm",
        root / "target/wasm32-wasip1/release/ml_wasi_kmeans.wasm",
        root / "mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_kmeans.wasm",
    ]
    ...
    cwasm_candidates += [
        root / "ml_wasi_kmeans.cwasm",
        root / "target/wasm32-wasip1/release/ml_wasi_kmeans.cwasm",
        root / "mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_kmeans.cwasm",
    ]
...
def main():
    ...
    metadata = {
        ...
        "epochs_compat": int(args.epochs),
        "lr_compat": float(args.lr),
    ...
    inertia_list, purity_list, iter_list = [], [], []
    ...
    while good_runs < max_runs and attempt < max_runs * 5:
        ...
        inertia_list.append(float(ml.get("inertia_mean", 0.0)))
        purity_list.append(float(ml.get("purity_mean", 0.0)))
        iter_list.append(float(ml.get("iterations_mean", ml.get("iters_mean", 0.0))))
    ...
    final = {
        ...
        "purity": safe_mean(purity_list, default=0.0),
        "inertia": safe_mean(inertia_list, default=0.0),
        "iterations_mean": safe_mean(iter_list, default=0.0),
