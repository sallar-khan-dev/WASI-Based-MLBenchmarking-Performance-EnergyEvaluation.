...
def resolve_runtime_binary(candidates: list[str | Path], label: str) -> str:
    for c in candidates:
        ...
        if found:
            return found
    raise SystemExit(
        f"Could not find binary for {label}. Checked: " + ", ".join(str(c) for c in candidates)
    )
...
def build_runtime_cmd(runtime: str, root: Path, wasm: Path, cwasm: Path | None, wasm_args: list[str]) -> list[str]:
    home = Path.home()
    ...
    if runtime == "wazero":
        wazero = resolve_runtime_binary(
            [root / "wazero/bin/wazero", root.parent / "wazero/bin/wazero", "wazero"],
            "wazero",
        )
    ...
    if runtime in ("wamr_interp", "wamr_fast_interp", "wamr_iwasm", "wamr_aot"):
        ...
        if runtime == "wamr_aot":
            aot_candidates = [
                root / "ml_wasi_dt_wamr.aot",
                root / "ml_wasi_dt.aot",
                root / "target/wasm32-wasip1/release/ml_wasi_dt.aot",
                root / "target/wasm32-wasip1/release/ml_wasi_dt_wamr.aot",
                root / "mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_dt.aot",
                root / "mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_dt_wamr.aot",
            ]
    ...
    if runtime == "wasm3":
        wasm3 = resolve_runtime_binary(
            [root.parent / "wasm3/build/wasm3", root.parent / "wasm3/build/m3", "wasm3", "m3"],
            "wasm3",
        )
    ...
    if runtime == "native":
        native_candidates = [
            root / "ml_native_dt",
            root / "target/release/ml_native_dt",
            root / "mlRust-wasi/target/release/ml_native_dt",
            root / "mlRust-native/target/release/ml_native_dt",
            root / "target/release/ml_wasi_dt",
        ]
        nb = first_existing(native_candidates)
        if nb is None:
            raise SystemExit("Native binary not found. Build it first before using --runtime native.")
...
def resolve_input_artifacts(root: Path, args) -> tuple[Path, Path]:
    wasm_candidates = []
    if args.wasm:
        wp = Path(args.wasm)
        wasm_candidates.append(wp if wp.is_absolute() else root / wp)
    wasm_candidates += [
        root / "ml_wasi_dt.wasm",
        root / "target/wasm32-wasip1/release/ml_wasi_dt.wasm",
        root / "mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_dt.wasm",
    ]
    wasm = first_existing([p.resolve() for p in wasm_candidates if p is not None])
    if wasm is None and args.runtime != "native":
        raise SystemExit(
            "Decision Tree WASM binary not found. Pass --wasm explicitly or place ml_wasi_dt.wasm "
            "under target/wasm32-wasip1/release/."
        )

    cwasm_candidates = []
    if args.cwasm:
        cp = Path(args.cwasm)
        cwasm_candidates.append(cp if cp.is_absolute() else root / cp)
    cwasm_candidates += [
        root / "ml_wasi_dt.cwasm",
        root / "target/wasm32-wasip1/release/ml_wasi_dt.cwasm",
        root / "mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_dt.cwasm",
    ]
    cwasm = first_existing([p.resolve() for p in cwasm_candidates if p is not None])
    if cwasm is None:
        cwasm = (root / (args.cwasm or "ml_wasi_dt.cwasm")).resolve()
...
def main():
    ...
    ap.add_argument("--resplit-each-trial", type=int, choices=[0, 1], default=1)
    ap.add_argument("--max-depth", type=int, default=8)
    ap.add_argument("--min-samples-split", type=int, default=32)
    ap.add_argument("--min-samples-leaf", type=int, default=8)
    ap.add_argument("--threshold-bins", type=int, default=8)
    ap.add_argument("--train-cap", type=int, default=4096)
    ...
    ap.add_argument("--wasm", type=str, default="ml_wasi_dt.wasm")
    ap.add_argument("--cwasm", type=str, default="ml_wasi_dt.cwasm")
    args = ap.parse_args()

    root = Path.cwd().resolve()
    wasm, cwasm = resolve_input_artifacts(root, args)
    ...
    common_args = [
        ...
        "--resplit-each-trial", str(args.resplit_each_trial),
        "--max-depth", str(args.max_depth),
        "--min-samples-split", str(args.min_samples_split),
        "--min-samples-leaf", str(args.min_samples_leaf),
        "--threshold-bins", str(args.threshold_bins),
        "--train-cap", str(args.train_cap),
    ]
    ...
    metadata = {
        ...
        "resplit_each_trial": int(args.resplit_each_trial),
        "max_depth": int(args.max_depth),
        "min_samples_split": int(args.min_samples_split),
        "min_samples_leaf": int(args.min_samples_leaf),
        "threshold_bins": int(args.threshold_bins),
    ...
    tree_nodes_list, tree_depth_list = [], []
    ...
    while good_runs < max_runs and attempt < max_runs * 5:
        ...
        tree_nodes_list.append(float(ml.get("tree_nodes_mean", 0.0)))
        tree_depth_list.append(float(ml.get("tree_depth_mean", 0.0)))
    ...
    final = {
        ...
        "tree_nodes_mean": safe_mean(tree_nodes_list, default=0.0),
        "tree_depth_mean": safe_mean(tree_depth_list, default=0.0),
