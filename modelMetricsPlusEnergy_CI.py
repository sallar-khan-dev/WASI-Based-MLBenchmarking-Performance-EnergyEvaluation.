...
def build_runtime_cmd(runtime: str, root: Path, wasm: Path, cwasm: Path | None, wasm_args: list[str]) -> list[str]:
    ...
    if runtime in ("wamr_interp", "wamr_fast_interp", "wamr_iwasm", "wamr_aot"):
        ...
        if runtime == "wamr_aot":
            aot_candidates = [
                root / "ml_wasi_lr_wamr.aot",
                root / "ml_wasi_lr.aot",
                root / "mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_lr.aot",
                root / "mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_lr_wamr.aot",
            ]
    ...
    if runtime == "native":
        native_candidates = [
            root / "ml_native_lr",
            root / "target/release/ml_native_lr",
            root / "mlRust-wasi/target/release/ml_native_lr",
            root / "mlRust-native/target/release/ml_native_lr",
        ]
...
def main():
    ...
    ap.add_argument("--wasm", type=str, default="mlRust-wasi/target/wasm32-wasip1/release/ml_wasi_lr.wasm")
    ap.add_argument("--cwasm", type=str, default="ml_wasi_lr.cwasm")
    args = ap.parse_args()
